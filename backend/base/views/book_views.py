...
from base.models import Book, Review, Category
from base.serializers import BookSerializer, CategorySerializer
...
def getDashboard(request):
    ...
    rating = {
        "ratingFive": books.filter(rating__gte=4, rating__lte=5).count(),
    ...
    novels = books.filter(genres__icontains="nonfiction").count()
...
def resetCategory(request):
    ...
    for i in range(len(dataset)):
        category = Category(name=dataset.iloc[i][1])
        category.save()

    return Response("Genres are added")
